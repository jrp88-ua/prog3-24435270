...
public class PlayerRandomPreTest {
	...
	/*
	 * Se crea un PlayerRandom (en el setUp) y se comprueba que el GameShip que se
	 * crea es del bando REBEL y que el nombre es correcto. Comprueba que el objeto
	...
	/*
	 * Se comprueba initFighters() para playerRandom. Para ello se constata que el
	 * fleet del GameShip asociado tiene 20 cazas y que el ship es correcto
	 ...
	 * comprueba que el fleet del GameShip asociado tiene 920 cazas y que el ship es
	 * correcto (coincide con kIMPERIALGAMESHIP)
	 */
	// TODO
	@Test
	public void testInitFightersImperial() {
		PlayerRandom player = new PlayerRandom(Side.IMPERIAL, 500);
		player.initFighters();
		assertEquals(920, player.getGameShip().getFleetTest().size());
		assertEquals(kIMPERIALGAMESHIP, player.getGameShip().toString());
	}

	/*
	 * Para un PlayerRandom sin iniciar (sin cazas en la nave) se comprueba que
	...
	/*
	 * Se comprueba showShip para playerRandom sin iniciarlo. Tras iniciarlo se
	...
	/*
	 * Comprueba purgeFleet() con un playerRandom con toda la flota de su nave
	 * intacta. El número de cazas debe coincidir con el que tenía antes de hacer
	 * purgeFleet. Destruye todos los cazas. Ejecuta purgeFleet(). Comprueba que
	 * ahora no existe ningún caza en la nave.
	 */
	// TODO
	@Test
	public void testPurgeFleet() {
		...
		assertEquals(20, playerRandom.getGameShip().getFleetTest().size());
		playerRandom.getGameShip().getFleetTest().forEach((f) -> f.addShield(-1000));
		playerRandom.purgeFleet();
		assertEquals(0, playerRandom.getGameShip().getFleetTest().size());
	}

	/*
	 * Se inicia playerRandom con cazas en su nave. Se le añade un tablero. Se
	 * invoca a nextPlay() varias veces de tal forma que entra en todas las opciones
	 * (excepto exit).
	 */
	@Test
	...
	/*
	 * Tras iniciar playerRandom con cazas y asignarles un tablero de 10x10, se
	 * invoca repetidamente nextPlay() hasta que retorne false. Se comprueba que el
	 * número de veces que se ejecutó es 105. Y además que el tablero con sus cazas
	...
	/*
	 * Realiza el test de comprobación de los parámetros null en PlayerRandom del
	 * constructor y de setBoard
	 */
	// TODO
	@Test
